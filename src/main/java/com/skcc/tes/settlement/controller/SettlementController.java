 ...
 */
@RestController
@RequiredArgsConstructor // pom.xml의 dependency로 lombok이 설정되어있어야 합니다. 아래 private final로 선언된 class를 초기화 하는데 사용됩니다.
public class SettlementController {

    private final SettlementRepository repository;

    // 생성
    @PostMapping("/settlement") // "@PostMapping" 은 "@RequestMapping(method = RequestMethod.POST)"을 축약한 것입니다.
    public SettlementDto saveNewSettlement(@RequestBody Settlement src) {
        /**
         * [ID를 null 값으로 설정해 준 이유]
         * JPA에서 엔티티의 ID 값이 null이거나 DB에 해당 ID값이 없으면 Insert를 실행하고, 이미 DB에 해당 ID 값이 있으면 Update를 실행합니다.
         * 그런데, Settlement 엔티티의 ID가 null 일때, 자동 생성되도록 설정되어 있습니다. "@GeneratedValue" <- 자동 생성
     ...
     * 구글에서 검색하면 오른쪽과 같은 예시를 만날 수 있습니다. https://www.google.com/search?q=hello
     * 여기서 q가
     *
     *
     * @RequestBody
